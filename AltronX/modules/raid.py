...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(mkraid) == 2:
            ...
            g = a.id
            if int(g) in ALTRON:
                await e.reply("» rishte me ye tumhare bap lgte h", parse_mode=None, link_preview=None)
            elif int(g) in SUDO_USERS:
                await e.reply("» ye to chapri hai", parse_mode=None, link_preview=None)
            else:
                c = a.first_name
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 11:
            ...
            if int(user_id) in ALTRON:
                ...
                await e.reply("» he is ur uncle bro", parse_mode=None, link_preview=None)
                ...
                phucker = [user_id]
                gey.append(phucker)
                await e.reply("» dolly bhn k lund ke nam par, replyraid activated !! ✅", parse_mode=None, link_preview=None)
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 12:
            ...
            try:
                ...
                pass
            await e.reply("» dolly bhai mana na krte to, muut deta tere muh p !! ✅", parse_mode=None, link_preview=None )
        elif e.reply_to_msg_id:             
            a = await e.get_reply_message()
            b = await e.client.get_entity(a.sender_id)
            g = b.id
