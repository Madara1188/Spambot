...
PythonHelp = f"★ ​𝑺𝑨𝑯𝑬𝑵𝑺𝑯𝑨𝑯 𝙃𝙚𝙡𝙥 𝙈𝙚𝙣𝙪 ★\n\n» **ᴄʟɪᴄᴋ ᴏɴ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴꜱ ꜰᴏʀ ʜᴇʟᴘ**\n» **ᴅᴇᴠᴇʟᴏᴘᴇʀ: @sahenshah01**"
...
async def help(event):
    if event.sender_id in SUDO_USERS:
       await event.client.send_file(event.chat_id,
                                  " https://graph.org/file/6b1e6a6b8b0bb4934689f.jpg",
                                  caption=PythonHelp,
                                  buttons=[
           [
...
async def helpback(event):
   if event.query.user_id in SUDO_USERS:    
      await event.edit(
        ...
        await event.answer("sudo k liye zor se bolo, dolly bhai ki maaki chut", cache_time=0, alert=True)
